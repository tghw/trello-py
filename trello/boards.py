...
class Boards(object):
    ...
    def get_action(self, board_id, entities=None, display=None, filter=None, fields=None, limit=None, format=None, since=None, before=None, page=None, idModels=None, member=None, member_fields=None, memberCreator=None, memberCreator_fields=None):
        resp = requests.get("https://trello.com/1/boards/%s/actions" % (board_id), params=dict(key=self._apikey, token=self._token, entities=entities, display=display, filter=filter, fields=fields, limit=limit, format=format, since=since, before=before, page=page, idModels=idModels, member=member, member_fields=member_fields, memberCreator=memberCreator, memberCreator_fields=memberCreator_fields), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)

    def get_boardStar(self, board_id, filter=None):
        resp = requests.get("https://trello.com/1/boards/%s/boardStars" % (board_id), params=dict(key=self._apikey, token=self._token, filter=filter), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def get_delta(self, board_id, tags, ixLastUpdate):
        resp = requests.get("https://trello.com/1/boards/%s/deltas" % (board_id), params=dict(key=self._apikey, token=self._token, tags=tags, ixLastUpdate=ixLastUpdate), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def get_label_idLabel(self, idLabel, board_id, fields=None):
        resp = requests.get("https://trello.com/1/boards/%s/labels/%s" % (board_id, idLabel), params=dict(key=self._apikey, token=self._token, fields=fields), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def get_membership(self, board_id, filter=None, member=None, member_fields=None):
        resp = requests.get("https://trello.com/1/boards/%s/memberships" % (board_id), params=dict(key=self._apikey, token=self._token, filter=filter, member=member, member_fields=member_fields), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)

    def get_membership_idMembership(self, idMembership, board_id, member=None, member_fields=None):
        resp = requests.get("https://trello.com/1/boards/%s/memberships/%s" % (board_id, idMembership), params=dict(key=self._apikey, token=self._token, member=member, member_fields=member_fields), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def get_pluginData(self, board_id):
        resp = requests.get("https://trello.com/1/boards/%s/pluginData" % (board_id), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)

    def update(self, board_id, name=None, desc=None, closed=None, subscribed=None, idOrganization=None, prefs/permissionLevel=None, prefs/selfJoin=None, prefs/cardCovers=None, prefs/invitations=None, prefs/voting=None, prefs/comments=None, prefs/background=None, prefs/cardAging=None, prefs/calendarFeedEnabled=None, labelNames/green=None, labelNames/yellow=None, labelNames/orange=None, labelNames/red=None, labelNames/purple=None, labelNames/blue=None):
        resp = requests.put("https://trello.com/1/boards/%s" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, desc=desc, closed=closed, subscribed=subscribed, idOrganization=idOrganization, prefs/permissionLevel=prefs/permissionLevel, prefs/selfJoin=prefs/selfJoin, prefs/cardCovers=prefs/cardCovers, prefs/invitations=prefs/invitations, prefs/voting=prefs/voting, prefs/comments=prefs/comments, prefs/background=prefs/background, prefs/cardAging=prefs/cardAging, prefs/calendarFeedEnabled=prefs/calendarFeedEnabled, labelNames/green=labelNames/green, labelNames/yellow=labelNames/yellow, labelNames/orange=labelNames/orange, labelNames/red=labelNames/red, labelNames/purple=labelNames/purple, labelNames/blue=labelNames/blue))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def update_labelName_blue(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/labelNames/blue" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_labelName_green(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/labelNames/green" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_labelName_orange(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/labelNames/orange" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_labelName_purple(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/labelNames/purple" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_labelName_red(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/labelNames/red" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_labelName_yellow(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/labelNames/yellow" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_member(self, board_id, email, fullName=None, type=None):
        resp = requests.put("https://trello.com/1/boards/%s/members" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(email=email, fullName=fullName, type=type))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def update_membership_idMembership(self, idMembership, board_id, type, member_fields=None):
        resp = requests.put("https://trello.com/1/boards/%s/memberships/%s" % (board_id, idMembership), params=dict(key=self._apikey, token=self._token), data=dict(type=type, member_fields=member_fields))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_emailPosition(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/emailPosition" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_idEmailList(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/idEmailList" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_showListGuide(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/showListGuide" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_showSidebar(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/showSidebar" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_showSidebarActivity(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/showSidebarActivity" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_showSidebarBoardAction(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/showSidebarBoardActions" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_myPref_showSidebarMember(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/myPrefs/showSidebarMembers" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def update_pref_background(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/prefs/background" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_pref_calendarFeedEnabled(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/prefs/calendarFeedEnabled" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_pref_cardAging(self, board_id, value):
        resp = requests.put("https://trello.com/1/boards/%s/prefs/cardAging" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def new(self, name, defaultLabels=None, defaultLists=None, desc=None, idOrganization=None, idBoardSource=None, keepFromSource=None, powerUps=None, prefs_permissionLevel=None, prefs_voting=None, prefs_comments=None, prefs_invitations=None, prefs_selfJoin=None, prefs_cardCovers=None, prefs_background=None, prefs_cardAging=None):
        resp = requests.post("https://trello.com/1/boards" % (), params=dict(key=self._apikey, token=self._token), data=dict(name=name, defaultLabels=defaultLabels, defaultLists=defaultLists, desc=desc, idOrganization=idOrganization, idBoardSource=idBoardSource, keepFromSource=keepFromSource, powerUps=powerUps, prefs_permissionLevel=prefs_permissionLevel, prefs_voting=prefs_voting, prefs_comments=prefs_comments, prefs_invitations=prefs_invitations, prefs_selfJoin=prefs_selfJoin, prefs_cardCovers=prefs_cardCovers, prefs_background=prefs_background, prefs_cardAging=prefs_cardAging))
        resp.raise_for_status()
        return json.loads(resp.content)

    def new_calendarKey_generate(self, board_id):
        resp = requests.post("https://trello.com/1/boards/%s/calendarKey/generate" % (board_id), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def new_emailKey_generate(self, board_id):
        resp = requests.post("https://trello.com/1/boards/%s/emailKey/generate" % (board_id), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)

    def new_label(self, board_id, name, color):
        resp = requests.post("https://trello.com/1/boards/%s/labels" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, color=color))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def new_powerUp(self, board_id, value):
        resp = requests.post("https://trello.com/1/boards/%s/powerUps" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(value=value))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def delete_powerUp_powerUp(self, powerUp, board_id):
        resp = requests.delete("https://trello.com/1/boards/%s/powerUps/%s" % (board_id, powerUp), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
